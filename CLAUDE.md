...
### REST
- `GET /routes` — lista de rutas con sus paradas (`id, name, mode, color_hex, stops: [{id, name, lat, lng, sequence}]`) y `shape` opcional: el trazado por calles de la ruta entera como pares `[lat, lng]` (falta sin OSRM, y en el teleférico, que va por el aire)
...
- `GET /stops/:id/eta` *(Escenario 2, agregado por el backend)* — ETA simple (`distance_km, eta_minutes`, ambos `null` si el vehículo de esa ruta aún no reportó posición); regla fija `distancia en línea recta / DEMO_VEHICLE_SPEED_KMH`, no sobre el trazado real de la ruta. Esa velocidad es la del **conductor simulado** (`SPEED_KMH`, 600 km/h por omisión), no la de un camión real: el ETA tiene que cuadrar con la unidad que se ve moverse en el mapa. Los minutos del itinerario (`GET /journeys`) sí van con la velocidad real por modo (`speedForMode`, 15 km/h combi/bus)
...
- `GET /walk-path` — trazado a pie por calles; query `from_lat, from_lng, to_lat, to_lng` y opcionalmente `via` (puntos intermedios, `lat,lng` separados por `;`) → `{ path: [[lat, lng], ...] }`, vacío cuando no hay trazado (nunca un error). Lo usan el tramo hasta la parada del flujo de camión y los modos de bici y caminata, que trazan en el cliente y pasan por `via` los vértices que no se pueden perder (el rodeo de una zona marcada, los extremos de una ciclovía)
- `GET /journeys` *(Escenario 8, agregado por el backend)* — recomendación de ruta con transbordos; query params `origin_lat, origin_lng, destination_lat, destination_lng` y opcionalmente `modes` (coma-separado: `bike,combi,bus,teleferico` — lista de modos *permitidos*; caminar siempre está disponible y no se filtra) → `{ alternatives: [...] }`. Sin `modes`, hasta 3 alternativas etiquetadas (`"Más rápida"`, `"Sin bicicleta"`, `"Con menos transbordos"`), sin duplicados; con `modes`, una sola (`"Tu selección"`) respetando esa restricción **y garantizando que el viaje use al menos uno de los modos pedidos** —"acepto combi" tiene que producir un viaje en combi, no la caminata completa porque salga antes; si con lo pedido no hay viaje posible (bici más allá de su límite de distancia), se cae al camino más rápido. Cada alternativa trae `legs` (cada `leg`: `mode`, `route_id/route_name` si aplica, `from/to`, `distance_km/eta_minutes`, y `geometry` opcional), `total_distance_km` y `total_eta_minutes`. `geometry` es el trazado por calles como pares `[lat, lng]`, que el servidor pide a OSRM (`OSRM_URL`, por omision el servidor publico de demo) despues de planear — **solo para dibujar**: `distance_km` y `eta_minutes` siguen siendo linea recta a velocidad fija, que es con lo que el motor eligio el viaje. Puede faltar (OSRM caido, tramo de longitud cero, teleferico que va por el aire) y entonces el cliente dibuja la recta entre extremos. Siempre hay al menos una alternativa (el peor caso es caminar todo el trayecto)
