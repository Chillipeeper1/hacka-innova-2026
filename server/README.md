...
npm run simulate      # en otra terminal: simula un conductor (ruta por defecto: id 1)
npm run simulate -- 2 # o especifica el routeId
...
(default `20000`, ver Escenario 5 abajo).
...
| POST | `/boarding-signals` | `{ user_id, stop_id, route_id, intent }` | `intent`: `boarding` (→ `waiting`) \| `passing` (→ `expired`) |
...
| POST | `/card-taps` (Escenario 6) | `{ card_uid, vehicle_id, tapped_at? }` | Confirma abordaje directo, salta `waiting` |
...
(`TRIP_DURATION_MS`, default 20s) que lo pasa a `alighted` si nadie lo
cambió de estado antes — además de la confirmación manual de descenso, que
sigue disponible vía `PATCH /boarding-signals/:id`.
